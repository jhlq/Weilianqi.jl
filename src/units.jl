...
#white unit is a spawn for all colors whereas any unit with color (1,1,1) "is" every color, so a white spawn is a spawn for all and "white" as a unit spec can be removed. White superpowers are being removed, "units" facilitating multiple color connections can be built with tunnel and bridge units 
#=
